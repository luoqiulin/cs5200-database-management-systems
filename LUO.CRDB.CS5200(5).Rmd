...
depPort VARCHAR(100) NOT NULL,
arrPort VARCHAR(100) NOT NULL,
airline VARCHAR(100) NOT NULL,
aircraft VARCHAR(100) NOT NULL,
state VARCHAR(100) NOT NULL,
flightPhase ENUM('takeoff','inflight','landing','unknown') NOT NULL,
impact VARCHAR(100) NOT NULL,
FOREIGN KEY (depPort,state) REFERENCES airports(name,state) ON DELETE CASCADE ON UPDATE CASCADE,
FOREIGN KEY (arrPort,state) REFERENCES airports(name,state) ON DELETE CASCADE ON UPDATE CASCADE,
FOREIGN KEY (airline) REFERENCES airlines(airline) ON DELETE CASCADE ON UPDATE CASCADE
);
```
...
/* Task 3 */
/* I choose to populate the unknown values as "N/A". */
/* Show table airports */
```{sql connection=dbcon}
...
/* Task 4 */
```{sql connection=dbcon}
SELECT airline, COUNT(*) AS numberStrike FROM incidents WHERE flightPhase = "landing" AND arrPort LIKE "%LaGuardia%" GROUP BY airline 
```

/* Task 5 */
/* It is assumed that neither "GOVERNMENT" nor "UNKNOWN" is a commercial airline. */
```{sql connection=dbcon}
SELECT COUNT(*) AS numberMaxStrike, arrPort AS airport FROM incidents WHERE airline <> "GOVERNMENT" AND airline <> "BUSINESS" AND airline <> "UNKNOWN" AND airline <> "PRIVATELY OWNED" AND airline <> "MILITARY" GROUP BY arrPort ORDER BY numberMaxStrike DESC LIMIT 1
```

/* Task 6 */
```{sql connection=dbcon}
SELECT COUNT(*) AS numberStrike, YEAR(str_to_date(idate,'%m/%d/%Y')) AS year FROM incidents WHERE YEAR(str_to_date(idate,'%m/%d/%Y')) <> 0 GROUP BY YEAR(str_to_date(idate,'%m/%d/%Y')) ORDER BY year 
```
